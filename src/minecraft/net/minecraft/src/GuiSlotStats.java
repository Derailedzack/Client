...
abstract class GuiSlotStats extends GuiSlot
{
    ...
    protected void func_27260_a(int i, int j, Tessellator tessellator)
    {
        ...
        if(field_27268_b == 0)
        {
            GuiStats.drawSprite(statsGui, (i + 115) - 18, j + 1, 0, 0);
        } else
        {
            GuiStats.drawSprite(statsGui, (i + 115) - 18, j + 1, 0, 18);
        }
        if(field_27268_b == 1)
        {
            GuiStats.drawSprite(statsGui, (i + 165) - 18, j + 1, 0, 0);
        } else
        {
            GuiStats.drawSprite(statsGui, (i + 165) - 18, j + 1, 0, 18);
        }
        if(field_27268_b == 2)
        {
            GuiStats.drawSprite(statsGui, (i + 215) - 18, j + 1, 0, 0);
        } else
        {
            GuiStats.drawSprite(statsGui, (i + 215) - 18, j + 1, 0, 18);
        }
        if(field_27271_e != -1)
        {
            char c = 'O';
            ...
            GuiStats.drawSprite(statsGui, i + c, j + 1, byte0, 0);
        }
    }
    ...
    protected void func_27265_a(StatCrafting statcrafting, int i, int j, boolean flag)
    {
        ...
        {
            String s = statcrafting.func_27084_a(GuiStats.getStatsFileWriter(statsGui).writeStat(statcrafting));
            statsGui.drawString(GuiStats.getFontRenderer4(statsGui), s, i - GuiStats.getFontRenderer5(statsGui).getStringWidth(s), j + 5, flag ? 0xffffff : 0x909090);
        } else
        {
            String s1 = "-";
            statsGui.drawString(GuiStats.getFontRenderer6(statsGui), s1, i - GuiStats.getFontRenderer7(statsGui).getStringWidth(s1), j + 5, flag ? 0xffffff : 0x909090);
        }
    }
    ...
    protected void func_27257_b(int i, int j)
    {
        ...
        } else
        {
            ...
            if(s.length() > 0)
            {
                int i1 = i + 12;
                int j1 = j - 12;
                int k1 = GuiStats.getFontRenderer8(statsGui).getStringWidth(s);
                GuiStats.drawGradientRect(statsGui, i1 - 3, j1 - 3, i1 + k1 + 3, j1 + 8 + 3, 0xc0000000, 0xc0000000);
                GuiStats.getFontRenderer9(statsGui).drawStringWithShadow(s, i1, j1, -1);
            }
        }
    }
    ...
    protected void func_27267_a(StatCrafting statcrafting, int i, int j)
    {
        ...
        if(s.length() > 0)
        {
            int k = i + 12;
            int l = j - 12;
            int i1 = GuiStats.getFontRenderer10(statsGui).getStringWidth(s);
            GuiStats.drawGradientRect1(statsGui, k - 3, l - 3, k + i1 + 3, l + 8 + 3, 0xc0000000, 0xc0000000);
            GuiStats.getFontRenderer11(statsGui).drawStringWithShadow(s, k, l, -1);
        }
    }
