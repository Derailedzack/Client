...
public class MapItemRenderer
{
    ...
    public void renderMap(EntityPlayer entityplayer, RenderEngine renderengine, MapData mapdata)
    {
        ...
        renderengine.func_28150_a(intArray, 128, 128, bufferedImage);
        int j = 0;
        int k = 0;
        ...
        for(Iterator iterator = mapdata.playersVisibleOnMap.iterator(); iterator.hasNext(); GL11.glPopMatrix())
        {
            ...
            float f1 = (float)(mapcoord.iconSize % 4 + 0) / 4F;
            float f2 = (float)(mapcoord.iconSize / 4 + 0) / 4F;
            float f3 = (float)(mapcoord.iconSize % 4 + 1) / 4F;
            float f4 = (float)(mapcoord.iconSize / 4 + 1) / 4F;
            tessellator.startDrawingQuads();
