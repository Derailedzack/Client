...
public class EffectRenderer
{
        ...
        for(int k1 = 0; k1 < j1; k1++)
        {
            for(int l1 = 0; l1 < j1; l1++)
            {
                for(int i2 = 0; i2 < j1; i2++)
                {
                    ...
                    addEffect((new EntityDiggingFX(worldObj, d, d1, d2, d - (double)i - 0.5D, d1 - (double)j - 0.5D, d2 - (double)k - 0.5D, block, j2, i1)).applyColourMultiplier(i, j, k));
                }
    ...
    public void addBlockHitEffects(int i, int j, int k, int l)
    {
        ...
        addEffect((new EntityDiggingFX(worldObj, d, d1, d2, 0.0D, 0.0D, 0.0D, block, l, worldObj.getBlockMetadata(i, j, k))).applyColourMultiplier(i, j, k).multiplyVelocity(0.2F).multipleParticleScaleBy(0.6F));
    }
