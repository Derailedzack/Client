...
public class BlockFire extends Block
{
    ...
    public void updateTick(World world, int i, int j, int k, Random random)
    {
        ...
        if(!flag && world.isRaining() && (world.canBlockBeRainedOn(i, j, k) || world.canBlockBeRainedOn(i - 1, j, k) || world.canBlockBeRainedOn(i + 1, j, k) || world.canBlockBeRainedOn(i, j, k - 1) || world.canBlockBeRainedOn(i, j, k + 1)))
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        }
        ...
        if(!flag && !canNeighborCatchFire(world, i, j, k))
        {
        ...
        for(int i1 = i - 1; i1 <= i + 1; i1++)
        {
            for(int j1 = k - 1; j1 <= k + 1; j1++)
            {
                for(int k1 = j - 1; k1 <= j + 4; k1++)
                {
                    ...
                    int j2 = (i2 + 40) / (l + 30);
                    if(j2 <= 0 || random.nextInt(l1) > j2 || world.isRaining() && world.canBlockBeRainedOn(i1, k1, j1) || world.canBlockBeRainedOn(i1 - 1, k1, k) || world.canBlockBeRainedOn(i1 + 1, k1, j1) || world.canBlockBeRainedOn(i1, k1, j1 - 1) || world.canBlockBeRainedOn(i1, k1, j1 + 1))
                    {
                        continue;
                    }
    ...
    private boolean canNeighborCatchFire(World world, int i, int j, int k)
    {
    ...
    {
        return world.isBlockNormalCube(i, j - 1, k) || canNeighborCatchFire(world, i, j, k);
    }

    public void onNeighborBlockChange(World world, int i, int j, int k, int l)
    {
        if(!world.isBlockNormalCube(i, j - 1, k) && !canNeighborCatchFire(world, i, j, k))
        {
            world.setBlockWithNotify(i, j, k, 0);
            return;
        } else
